...
apk update
apk verify /tmp/owrtpc-0.1.0-r1.apk
apk verify /tmp/luci-app-owrtpc-0.1.0-r1.apk
```
...
```sh
apk add /tmp/owrtpc-0.1.0-r1.apk
# Optional:
apk add /tmp/luci-app-owrtpc-0.1.0-r1.apk
```
...
upload `owrtpc-0.1.0-r1.apk`. Wait for successful installation and
resolution of its dependencies. Then upload
`luci-app-owrtpc-0.1.0-r1.apk` in a second operation. Refresh LuCI and
